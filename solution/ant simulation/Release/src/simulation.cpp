...
namespace sim {
    ...
    //shaders and their buffers
    GLuint mainShaderId;
    GLuint overlayShaderId;

    GLuint midCompShader;
    GLuint bounceCompShader;
    GLuint XDCompShader;
    GLuint infCompShader;

    GLuint computeTex;

    GLuint computeframebuffer;
    GLuint computeframebuffertex;

    GLuint overlayframebuffer;
    GLuint overlayframebuffertex;
    ...
    void setup() {
        ...
        createShader("src/shaders/mainFrag.frag", &mainShaderId);
        createShader("src/shaders/overlayFrag.frag", &overlayShaderId);
        createComputeShader("src/shaders/XDComputeShader.comp", &XDCompShader);
        createComputeShader("src/shaders/bounceComputeShader.comp", &bounceCompShader);
        createComputeShader("src/shaders/midComputeShader.comp", &midCompShader);
        createComputeShader("src/shaders/infiniteComputeShader.comp", &infCompShader);
        createCShaderBuffers();
        createFrameBuffer(&computeframebuffer, &computeframebuffertex);
        createFrameBuffer(&overlayframebuffer, &overlayframebuffertex);

        currShader = midCompShader;
    ...
    void compute(GLFWwindow* window) {
        ...
        glUniform1i(1, computeframebuffertex);
        ...
        glUniform1i(5, computeframebuffertex);
        glUniform3f(3, decay / 1000.0f, 0.0f, 0.0f);

        glActiveTexture(GL_TEXTURE0 + computeTex);
        glBindTexture(GL_TEXTURE_2D, computeTex);
        glActiveTexture(GL_TEXTURE0 + computeframebuffertex);
        glBindTexture(GL_TEXTURE_2D, computeframebuffertex);
    ...
    void renderGUI(ImGuiIO io, GLFWwindow* window, int * updateSpeed) {
        ...
        ImGui::Begin("simulation", 0, ImGuiWindowFlags_NoResize | ImGuiWindowFlags_NoNav | ImGuiWindowFlags_NoCollapse);
        {
            ...
            if (useOverlay)
                ImGui::Image((ImTextureID)overlayframebuffertex, wsize, ImVec2(0, 1), ImVec2(1, 0));
            else
                ImGui::Image((ImTextureID)computeframebuffertex, wsize, ImVec2(0, 1), ImVec2(1, 0));
            ImGui::EndChild();
        }
        ImGui::End();
        ...
        if (ImGui::IsItemHovered())
            ImGui::Text("inverts colors, or you can put in your own shader to make it prettier");
        ...
        ImGui::Text("different compute shaders");
        if (ImGui::Button("agents teleport to middle when offscreen"))
            currShader = midCompShader;        
        if (ImGui::Button("agents bounce of walls"))
            currShader = bounceCompShader;        
        if (ImGui::Button("hm"))
            currShader = XDCompShader;
    ...
    void createShader(const char* path, GLuint *id) {
        ...
        const char* vertexPath = "src/shaders/quadVert.vert";
